...
(function ($) {
  ...
  var settings = {}, roots = {}, caches = {},
    ...
    view = {
      getFilePath: function (setting, node) {
        var path = node.getPath();
        var filepath = path.map((file) => {
          return file.name;
        })
      ...
      makeNodeIcoClass: function (setting, node) {
        ...
        if (!node.isAjaxing) {
          ...
          if (isParent) { //文件夹图标,在css中定义图标路径和样式位置
            icoCss.push(node.open ? consts.folder.OPEN : consts.folder.CLOSE);
          } else { //文件图标，在css中定义其位置，图标使用node.icon属性定义其路径（makeNodeIcoStyle）
