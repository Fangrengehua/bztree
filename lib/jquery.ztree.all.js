...
(function (_$) {
  ...
  _initNode = function _initNode(setting, level, n, parentNode, isFirstNode, isLastNode, openFlag) {
    ...
    /**
     * 2020.10.30
     * add path to node By Linjin Qiao
     * setFilePath (n, filepath)
     * n.path
     */
    var setFilePath = function setFilePath(n, filepath) {
      var pId = n.parentTId;
      if (pId === null) {
        ...
        filepath.unshift('/' + n.name);
        n = data.getNodeCache(setting, n.parentTId);
  ...
  view = {
    getFilePath: function getFilePath(setting, node) {
      var path = node.getPath();
      var filepath = path.map(function (file) {
        return file.name;
      });
      filepath = filepath.join('/');
      return filepath.unshift('/');
    },
      ...
      if (!node.isAjaxing) {
        ...
        if (isParent) {
          ...
          if (FileType.length > 1) {
            ...
            if (consts.folder.IMAGE.indexOf(type) > -1) {
              //image
              icoCss.push("image");
            } else if (consts.folder.VIDEO.indexOf(type) > -1) {
              //video
              icoCss.push("video");
            } else if (consts.folder.AUDIO.indexOf(type) > -1) {
              //audio
              icoCss.push("audio");
            } else if (consts.folder.TXT.indexOf(type) > -1) {
              //audio
              icoCss.push("text");
            } else if (consts.folder.TS.indexOf(type) > -1) {
              //typescript
              icoCss.push("ts");
            } else {
              //其他                       
    ...
    makeNodeIcoStyle: function makeNodeIcoStyle(setting, node) {
      ...
      // 			if (consts.folder.IMAGE.indexOf(type) > -1) { //image
      // 				icoStyle.push("file_type_image.svg)");
      // 			} else if (consts.folder.VIDEO.indexOf(type) > -1) { //video
      // 				icoStyle.push("file_type_video.svg)");
      // 			} else if (consts.folder.AUDIO.indexOf(type) > -1) { //audio
      // 				icoStyle.push("file_type_audio.svg)");
      // 			} else if (consts.folder.TXT.indexOf(type) > -1) { //audio
      // 				icoStyle.push("file_type_text.svg)");
      // 			} else { //其他                       
      // 				icoStyle.push("file_type_", type, ".svg)");
      // 			}
      // 		} else { //无后缀名文件
      // 			var type = FileType.shift();
      // 			if (type.toLowerCase() == "license") {
      // 				icoStyle.push("file_type_license.svg)");
      // 			}
      // 			else { icoStyle.push("default_file.svg"); }
      // 		}
      // 	}
      // }
    ...
    setNodeName: function setNodeName(setting, node) {
      //var title = data.nodeTitle(setting, node),
      this.nObj = $$(node, consts.id.SPAN, setting);
      this.nObj.empty();
      if (setting.view.nameIsHTML) {
        this.nObj.html(data.nodeName(setting, node));
      } else {
        this.nObj.text(data.nodeName(setting, node));
      }
